		     ...
		     void *netfs_priv)
{
	...
	rreq = netfs_alloc_read_request(ops, ractl->file);
	if (!rreq)
		goto cleanup;
		   ...
		   void *netfs_priv)
{
	...
	rreq = netfs_alloc_read_request(ops, file);
	if (!rreq) {
		if (netfs_priv)
...
retry:
	 ...
	 */
	if (!ops->is_cache_enabled(file_inode(file)) &&
	    round_down(pos, thp_size(page)) >= i_size_read(file_inode(file))) {
