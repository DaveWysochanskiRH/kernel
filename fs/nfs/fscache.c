...
{
	struct file *file = subreq->rreq->file;
	...
	dfprintk(FSCACHE,
		 "NFS: nfs_issue_op(fsc:%p s:%lu l:%lu) subreq->start: %lld subreq->len: %ld subreq->transferred: %ld\n",
		 nfs_i_fscache(inode), start, last, subreq->start, subreq->len, subreq->transferred);
	...
	xas_for_each(&xas, page, last) {
		...
		/* FIXME: Skip nfs_pageio_complete_read if error ? */
		if (subreq->error < 0)
			break;
	}
	rcu_read_unlock();
...
static bool nfs_clamp_length(struct netfs_read_subrequest *subreq)
{
	...
	if (subreq->len > rsize) {
		dfprintk(FSCACHE,
			 "NFS: nfs_clamp_length(fsc:%p slen:%lu rsize: %u)\n",
			 nfs_i_fscache(inode), subreq->len, rsize);
		subreq->len = rsize;
	}
...
static void nfs_init_rreq(struct netfs_read_request *rreq)
{
	struct nfs_inode *nfsi = NFS_I(file_inode(rreq->file));
