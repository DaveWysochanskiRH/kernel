...
static void nfs_issue_op(struct netfs_read_subrequest *sreq)
{
	...
	desc.ctx = sreq->rreq->netfs_priv;

	dfprintk(FSCACHE, "NFS: %s(fsc:%p s:%lu l:%lu) sreq->start: %lld "
		 "sreq->len: %ld sreq->transferred: %ld\n",
		 __func__, nfs_i_fscache(inode), start, last, sreq->start,
		 sreq->len, sreq->transferred);
	...
	desc.pgio.pg_sreq = sreq; /* used in completion */

	rcu_read_lock();
	xas_for_each(&xas, page, last) {
		sreq->error = readpage_async_filler(&desc, page);
		if (sreq->error < 0)
			break;
	}
	rcu_read_unlock();
...
static bool nfs_clamp_length(struct netfs_read_subrequest *sreq)
{
	struct inode *inode = sreq->rreq->mapping->host;
	unsigned int rsize = NFS_SB(inode->i_sb)->rsize;

	if (sreq->len > rsize) {
		dfprintk(FSCACHE,
			 "NFS: %s(fsc:%p slen:%lu rsize: %u)\n",
			 __func__, nfs_i_fscache(inode), sreq->len, rsize);
		sreq->len = rsize;
	}
			      ...
			      struct page *page)
{
	...
	ret = netfs_readpage(file, page, &nfs_fscache_req_ops, ctx);
 ...
 */
int nfs_readahead_from_fscache(struct nfs_open_context *ctx,
			       struct readahead_control *ractl)
{
	if (!NFS_I(ractl->mapping->host)->fscache)
		return -ENOBUFS;

	netfs_readahead(ractl, &nfs_fscache_req_ops, ctx);
...
void nfs_read_completion_to_fscache(struct nfs_pgio_header *hdr,
				    unsigned long bytes)
{
	struct netfs_read_subrequest *sreq = hdr->sreq;

	if (NFS_I(hdr->inode)->fscache && sreq) {
		hdr->sreq = NULL;
		dfprintk(FSCACHE,
			 "NFS: read_completion_to_fscache(fsc:%p err:%d bytes:%lu sreq->len:%lu\n",
			 NFS_I(hdr->inode)->fscache, hdr->error, bytes, sreq->len);
		netfs_subreq_terminated(sreq, hdr->error ?: bytes, false);
	}
}
