...
static void cachefiles_read_complete(struct kiocb *iocb, long ret, long ret2)
{
	...
	if (ret < 0) {
		req->error = ret;
	} else if (ret != req->len - req->transferred) {
		/* We got a short read.  The caller might want to try seeking
		 * ahead from this point.
		 */
		if (ret > 0 || !test_bit(FSCACHE_IO_SEEK_DATA_READ, &req->flags)) {
			_debug("short %lx %llx\n", ret, req->len);
			req->transferred += ret;
			__set_bit(FSCACHE_IO_SHORT_READ, &req->flags);
		} else {
			/* We didn't make any progress on a second try. */
			_debug("no data %lx %llx\n", ret, req->len);
			req->error = -ENODATA;
		}
		__clear_bit(FSCACHE_IO_SEEK_DATA_READ, &req->flags);
	} else {
		req->transferred += ret;
		__set_bit(FSCACHE_IO_DATA_FROM_CACHE, &req->flags);
	}
	if (req->io_done)
		req->io_done(req);
		    ...
		    struct iov_iter *iter)
{
	 ...
	 */
	if (test_bit(FSCACHE_IO_SEEK_DATA_READ, &req->flags)) {
		loff_t off = req->pos + req->transferred, off2;
		size_t gap;

		off2 = vfs_llseek(file, off, SEEK_DATA);
		if (IS_ERR_VALUE(off2) && off2 != -ENXIO) {
			ret = off2;
			goto presubmission_error;
		}

		if (off2 == -ENXIO || off2 >= req->pos + req->len) {
			/* The region is beyond the EOF or there's no more data
			 * in the region, so clear the rest of the buffer and
			 * return success.
			 */
			gap = req->len - req->transferred;
			iov_iter_zero(gap, iter);
			req->transferred += gap;
			__set_bit(FSCACHE_IO_DATA_FROM_CACHE, &req->flags);
			__clear_bit(FSCACHE_IO_SEEK_DATA_READ, &req->flags);
			fscache_end_io_operation(req->cookie);
	...
	ki->iocb.ki_pos		= req->pos + req->transferred;
